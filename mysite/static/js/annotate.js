...
const nerButton = document.getElementById('send-to-model');
...
nerButton.addEventListener('click', () => {
    // 建立 FormData 物件，並將檔案加入其中
    let formData = new FormData();
    formData.append("file", file);
    console.log(file);
    // 使用 XMLHttpRequest 送出
    ...
    xhttp.open("POST", "test/", true);  // 將 'my_view_url' 替換為你的視圖 URL 名稱
    xhttp.send(formData);
